...
<head>
  <meta charset="utf-8">
  <title>Factura de compra <?= htmlspecialchars($factura['num_doc_compra']); ?></title>
...
<div class="container factura-box">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <div>
      ...
      <div>INVENTARIO PEPS · COMPRAS</div>
    </div>
    <div class="text-end">
      <div><strong>Factura de Compra:</strong> <?= htmlspecialchars($factura['num_doc_compra']); ?></div>
  ...
  <div class="row mb-3">
    <div class="col-md-6">
      <h5 class="text-success">Proveedor</h5>
      <div><strong><?= htmlspecialchars($factura['proveedor_nombre'] ?? 'N/D'); ?></strong></div>
      <?php if (!empty($factura['contacto_nombre'])): ?>
        <div>Contacto: <?= htmlspecialchars($factura['contacto_nombre']); ?></div>
      <?php endif; ?>
  ...
  <div class="table-responsive mb-3">
    <table class="table table-dark table-bordered align-middle text-center">
      ...
      <?php 
        $gran_total = 0;
        foreach ($items as $it): 
          $subtotal = floatval($it['cantidad']) * floatval($it['costo_unitario']);
          $gran_total += $subtotal;
      ?>
        <tr>
          ...
          <td>$ <?= number_format($it['costo_unitario'], 4); ?></td>
  ...
  <div class="d-flex justify-content-between align-items-center mt-4">
    ...
    <a href="index.php?controller=movimientos&action=entrada" class="btn btn-volver">
      Volver a compras
    </a>
  </div>
</div>
